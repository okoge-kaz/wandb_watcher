...
def get_current_running(entity: str, project: str, run_id: str) -> Optional[Run]:
    runs: list[Run] = wandb.Api().runs(
        path=f"{entity}/{project}",
    )
    current_running: Optional[Run] = None

    for run in runs:
        if run.state == "running" and run.id == run_id:
            current_running = run
            break
...
def arg_parser() -> argparse.Namespace:
    ...
    parser.add_argument("--entity", type=str, required=False, default="llm-jp")
    parser.add_argument("--project", type=str, required=False, default="MDS-175B-2023-1005")
    parser.add_argument("--run-id", type=str, required=False, default="")
...
def main() -> None:
    ...
    run = get_current_running(entity=args.entity, project=args.project, run_id=args.run_id)
    if run is None:
        raise ValueError("No running run found")
    print(f"LOG: currently running: {run.id}")
